...
public class Shoulder extends Subsystem {
  private static final DoubleUnaryOperator gravityFunc = Values.mapRange(-500, -5000, -0.03, -0.235);
  public TalonSRX shoulder = new TalonSRX(RobotMap.SHOULDER_PORT);
    public static enum ShoulderPreset{
      ...
      // UP IS MORE NEGATIVE
        ...
        GROUND_BALL(-20), //was 3745+180-220-180
        GROUND_HATCH(-10),
        TRAVEL(-4870),
  
        PLACE_TRAVEL(-5010),
  
        FEEDER_STATION_BALL(-4870),
        FEEDER_STATION_HATCH(-870), //was 1601
      
        ROCKET_LOW_HATCH(-870),

        ROCKET_MID_HATCH(-4080),
        ROCKET_HIGH_HATCH(-4100),
  
        CARGOSHIP_BALL(1060-3069),
  
        ROCKET_LOW_BALL(1313+75-3069),
        ROCKET_MID_BALL(-4540),
        ROCKET_HIGH_BALL(-4430),
      
  
         HATCH_GROUND_2_ELEVATOR_TRANSFER(-600-3069);
  ...
  public void configMotorController(int timeout){
    //for empty/ball
    shoulder.config_kP(0, 15, timeout);
    shoulder.config_kI(0, 0, timeout);
    shoulder.config_kD(0, 220, timeout);
    shoulder.config_kF(0, 5.0, timeout);
    shoulder.config_IntegralZone(0, 0, timeout);

    shoulder.configMotionAcceleration(900, timeout); //was 400
    shoulder.configMotionCruiseVelocity(600, timeout); //was 400
